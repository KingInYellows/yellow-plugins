...
| `dismissed`  | Won't-fix or false positive, with actor and reason. **Suppresses re-raising while the anchored code and every recorded `depends_on` hash are unchanged.** |
...
| `reopened`   | A fixed finding reproduced again at a new head, or a dismissed one whose anchored code or any `depends_on` hash changed                                   |
