...
**Date:** 2026-05-08 (revision 2: 2026-08-01, re-enriched via `/yellow-research:workflows:deepen-plan`; that command is `/yellow-research:flow:deepen-plan` since the 2026-08-10 namespace rename)
...
> **⚠️ SUPERSEDED (2026-08-01 rescope brainstorm) — do not implement this phase as written.** `docs/brainstorms/2026-08-01-yellow-council-v2-rescope-brainstorm.md` cut Phase 3 down to error-string detection only: only **Task 3.3** (the `QUOTA_EXHAUSTED` verdict + per-reviewer error-string detection) survives into V2, ratified as spec requirements **R16–R18** in `plans/specs/yellow-council-v2-four-cli.md` and implemented in `plans/shells/yellow-council-v2-four-cli-04-quota-and-opencode-routing.md`. **Task 3.1** (`quota.json` state file, dual-window invariants), **Task 3.2** (pre-flight headroom check), **Task 3.4** (the numeric-tracking half of the documentation task), **Task 3.5** (`COUNCIL_QUOTA_RESET` handler), and the **`COUNCIL_MIN_REVIEWERS` quorum gate** were CUT from V2 entirely — not deferred to a later version. Do not build any of that scaffolding from the sections below; they're kept only as historical context for why the slim design was chosen.
...
PR-0: codex-reviewer contract normalization (Phase 0)       ← stack prerequisite (yellow-codex)
  └─ PR-A: claude-reviewer agent + 4-way fan-out (Phase 1)  ← foundation (yellow-council)
       └─ PR-B: double-blind + 2-pass + rubric synthesis     ← Phase 2 (depends on A)
            └─ PR-C: subscription quota tracking              ← Phase 3 (orthogonal*) (superseded — slimmed to error-string detection only, see brainstorm)
...
| Phase 3: Quota tracking | 5 tasks | ~1 day (superseded — see brainstorm; only Task 3.3 ships) |
...
- ✅ Minimum quorum: `COUNCIL_MIN_REVIEWERS=2` default; preflight surfaces AskUserQuestion when active reviewers drop below this threshold — **⚠️ SUPERSEDED:** cut from V2 entirely by `docs/brainstorms/2026-08-01-yellow-council-v2-rescope-brainstorm.md` along with the rest of the numeric quota scaffolding (Tasks 3.1/3.2/3.4/3.5); see the Phase 3 banner above
...
1. **Synthesis as separate Task subagent vs. inline orchestrator logic?** Plan keeps inline (simpler). Pulling synthesis into a dedicated `synthesis-agent` Task gives cleaner double-blind boundary at the cost of an extra Claude message. Defer to V3 unless empirical bias measurements show inline synthesis is leaking lineage labels. (CARE-style correlated-judge aggregation is the V3-grade version of this.) — **RESOLVED at spec time:** `plans/specs/yellow-council-v2-four-cli.md`'s Design section ratifies this record's lean — synthesis stays inline orchestrator logic in V2, a dedicated synthesis Task subagent explicitly rejected, revisit only if post-V2 bias measurements show inline synthesis leaking lineage labels. No longer open.
...
External research sources used by `/yellow-research:workflows:deepen-plan` (revision 2, 2026-08-01; that command is `/yellow-research:flow:deepen-plan` since the 2026-08-10 namespace rename; supersedes the May 2026 source list):
