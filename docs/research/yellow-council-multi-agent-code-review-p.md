...
> **Status: prior research, partially superseded.** This is the original 10-CLI exploration. For implementation, the **4-CLI subscription-auth research** (`multi-cli-code-review-claude-codex-gemini-opencode.md`), the **V2 spec** (`plans/specs/yellow-council-v2-four-cli.md`), and the **V2 revalidation record** (`docs/research/yellow-council-v2-revalidation-2026-08-01.md`, "Open decisions to surface in PRs" → "Locked by user" section) are authoritative, specifically:
...
>
> This file remains in the repo for historical context (research methodology, broader CLI survey, bias-mitigation literature review) but should not be used to make implementation choices that conflict with the locked decisions above.
