...
**Severity**: ERROR **Category**: SCHEMA_VALIDATION **Spec Reference**: FR-001,
FR-002

**Description**: The provided data is not valid JSON and cannot be parsed.
...
**Description**: A required field is missing from the manifest or marketplace
file.
...
**Description**: A field value does not match the expected format (e.g., URI,
email, date-time, semver).
...
- **Identifiers**: Use lowercase letters, numbers, and hyphens only (e.g.,
  `my-plugin`)
...
**Description**: A field value has the wrong data type (e.g., string instead of
number).
...
**Description**: A field value violates schema constraints (minLength,
...
**Description**: The manifest contains fields not defined in the schema
...
**Description**: A field value is not one of the allowed enumeration values.

**Common Causes**:

- Using invalid category (must be one of: development, productivity, security,
  learning, testing, design, database, deployment, monitoring)
...
**Severity**: ERROR **Category**: COMPATIBILITY **Spec Reference**: CRIT-002b

**Description**: The plugin requires a newer version of Claude Code than
currently installed.
...
**Description**: The plugin does not support your Claude Code version (uses
deprecated APIs).
...
**Severity**: ERROR **Category**: COMPATIBILITY **Spec Reference**: CRIT-005

**Description**: The plugin requires a newer Node.js version than currently
installed.
...
**Description**: The plugin does not support your operating system.
...
**Description**: The plugin does not support your CPU architecture.
...
**Description**: The plugin requires other plugins to be installed first.
...
**Severity**: ERROR **Category**: INSTALLATION **Spec Reference**: CRIT-007

**Description**: The requested plugin ID does not exist in the marketplace.
...
**Description**: The requested plugin version does not exist.
...
**Description**: The plugin is already installed.
...
**Description**: Failed to download plugin files from source.
...
**Description**: Downloaded plugin files do not match expected checksum
(security risk).
...
**Severity**: ERROR **Category**: INSTALLATION **Spec Reference**: CRIT-010

**Description**: A lifecycle script (preInstall, install, postInstall) failed
during execution.
...
**Description**: The plugin manifest file (plugin.json) is invalid or corrupted.
...
(permissions, disk I/O, pinned entries).
...
**Severity**: ERROR **Category**: DISCOVERY **Spec Reference**: CRIT-008

**Description**: The marketplace.json file does not exist or is not accessible.
...
**Description**: The marketplace.json file fails schema validation.
...
**Description**: A plugin entry in marketplace.json points to a non-existent
manifest.
...
**Severity**: WARNING (non-blocking) **Category**: DISCOVERY **Spec Reference**:
CRIT-008

**Description**: The plugin's changelog URL is unreachable or returns an error.
...
**Severity**: ERROR **Category**: PERMISSION **Spec Reference**: CRIT-012

**Description**: Plugin attempts to access resources without declaring required
permissions.
...
**Description**: Permission scope is not one of the allowed values.
...
**Description**: User must explicitly consent to plugin permissions before
installation.
...
**Severity**: ERROR **Category**: NETWORK **Spec Reference**: CRIT-011

**Description**: Failed to fetch data from network (marketplace, plugin,
changelog).
...
**Description**: Downloaded data could not be parsed (invalid JSON, corrupted
data).
...
2. **Test Compatibility**: Verify across Claude Code versions, Node versions,
   platforms
3. **Document Permissions**: Clearly explain why permissions are needed
4. **Handle Errors Gracefully**: Provide helpful error messages in lifecycle
   scripts
