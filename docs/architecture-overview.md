...
| Raw `git push` in Bash              | stacked-PR provider `PreToolUse` hook denies it (exit 2); a command it cannot tokenize is denied, but a malformed hook envelope is allowed                                          |
