...
| 5   | Write-time dedup (fingerprint: file, normalized category, rule, canonical enclosing scope, code-context hash; line kept as a rematch hint and keyed only for `unscoped` findings; reviewer recorded but not keyed) + dismissed-findings prompt injection | `rule` (closed per-category vocabulary) separates distinct defects on the same statement; canonical scope keeps identical code in two enclosing symbols separate; the code-context hash merges cross-reviewer repeats of the same anchored defect. Leaving line numbers out keeps identity stable when unrelated edits shift lines. Fingerprint alone still misses reworded re-detections (LLM titles vary run to run); prompt injection closes that gap using the fenced-advisory pattern already used for learnings-context. Plan must define the initial `rule` vocabulary and test that different `rule` values stay separate while the same `rule` from two reviewers merges. |
...
1. **Ledger library + schema** — `plugins/yellow-review/lib/review-ledger.sh`
   ...
   than rejected; credential redaction of every persisted model-authored string
   (`title`, `suggested_fix`, `scope`, dismissal reasons and any other
   model-produced field) and every anchor-snapshot line before it is appended,
   with the same patterns as yellow-core's `redact_secrets`
