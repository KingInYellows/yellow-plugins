...
**Selected approach:** B — Belt and Suspenders (Setup Fix + Workflow Hardening)
...
pattern to absorb cold-start latency, and introducing a retry-once pattern in
the canonical integration skill so all consuming workflows inherit resilience.
...
With npx-only, 3 of 5 hooks are guaranteed to fail, and a 4th is marginal.
This means memory injection into prompts, pre-edit context, and post-edit
recording have been non-functional.
...
logic -- a single failure causes the workflow to skip memory operations entirely.

## Setup Command Gap

The current `/ruvector:setup` command (setup.md):
- Step 2a runs `install.sh` which does `npm install -g ruvector --ignore-scripts`
...
- The setup command warns about degraded performance but does not treat a missing
  global binary as a setup failure
...
**Best when:** npx latency is the only cause and MCP reliability is already fine.
...
setup ensures global binary, workflows absorb MCP cold start via warmup, and
a retry-once pattern handles transient MCP failures.

**Pros:**
- Addresses all three root causes: npx latency, MCP cold start, MCP execution failures
...
- Changes to `mcp-integration-patterns` propagate to all workflows automatically
- Global binary makes ALL 5 hooks functional

**Cons:**
- Larger change surface -- touches setup, install script, integration skill, and workflow commands
...
responds in 81ms. This makes hooks reliable AND gives the MCP server a warm
CLI backend. The warmup pattern (`hooks_capabilities`) absorbs the remaining
MCP cold-start penalty, and retry-once handles the rare server crash.
...
either resolves immediately or is a hard failure. If retry fails, skip
gracefully (existing behavior).

### 4. Update mcp-integration-patterns skill as the single source of truth

Rather than patching each workflow command individually, update the canonical
patterns in `mcp-integration-patterns` SKILL.md. Consuming commands
(brainstorm, plan, work, compound, review-pr, resolve-pr) reference this skill
and should inherit the new patterns. This reduces maintenance burden and ensures
consistency.

### 5. Capability audit informs follow-up, not this implementation

The audit of 100+ unused ruvector tools is documented here for reference but
is explicitly out of scope for this implementation. A separate brainstorm will
...
1. **Update `install.sh`:**
   ...
   - If `~/.local/bin` is not in PATH, add it to shell profile and export
...
2. **Update `setup.md`:**
   ...
   - Add smoke test: run `timeout 1 ruvector hooks recall --top-k 1 "setup-test"`
     and verify it completes within 1 second
...
3. **Update `mcp-integration-patterns` SKILL.md Pattern 1 (Recall-Before-Act):**
   - Add warmup step between ToolSearch and the real call:
     ...
     ...
     ```
   - Document: warmup absorbs MCP cold start (300-1500ms). The capabilities
     call is idempotent and returns engine status.
...
4. **Update `mcp-integration-patterns` SKILL.md Pattern 1 execution:**
   - Change step 2 (MCP execution error handling) to:
     ...
     2. If MCP execution error on first attempt: pause 500ms, retry once.
        If retry also fails: note warning, skip to next step.
     ```
...
5. **Verify consuming commands reference the skill patterns:**
   ...
   - Update any that inline the pattern rather than referencing the skill
...
Suggested command: `/workflows:brainstorm ruvector capability expansion -- which unused tools to surface`
