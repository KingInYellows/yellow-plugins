...
| **Operational Runbook (`docs/operations/runbook.md`)** | FR-001 – FR-013 (diagnostic support), FR-009 (Publish workflow recovery) | NFR-REL-002 (Rollback recovery), NFR-REL-004 (Schema validation recovery), NFR-MAINT-002 (CI troubleshooting) | Architecture §3.7 operational procedures implementation with step-by-step incident response, diagnostics commands, failure recovery procedures (registry corruption, cache recovery, CI failures, security audit), and escalation paths. Task I4.T3 deliverable. |
