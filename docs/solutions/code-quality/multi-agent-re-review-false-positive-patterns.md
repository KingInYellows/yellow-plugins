...
### Calibration check: the bounded one-extra-round policy held across a 6-PR batch

A `/review:sweep-all` batch spanning PRs #676–#681 hit the fix-begets-finding
resolve loop twice — #676 and #677 each stopped their automatic resolve loop
at round 2 per the bound above. In both cases the round-2 findings were
real, resolve-worthy issues (not noise generated by the bound stopping too
early, and not a case where the bound let a genuine unbounded loop run
longer than 2 rounds). This is confirming evidence, not a new failure mode:
two independent PRs in the same batch both validated that round 2 is the
right stopping point rather than a number chosen without feedback. No
change to the policy is indicated by this batch — recorded here so a future
review of this doc has a second and third data point beyond PR #647 and
#664 without needing to re-derive them from session transcripts.
