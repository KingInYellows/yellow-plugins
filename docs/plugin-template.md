...
in Step 5 — do not hand-edit). `name` comes from the plugin's directory name
(matched against `pluginOrder` in `catalog/catalog.json`) and `version` from
`plugins/my-plugin/package.json`; author `description` and `author` in
`catalog/plugins/my-plugin.json` instead (field rules in `catalog/README.md`).
...
3. From the workspace root, run `pnpm generate:manifests` (if you are still in
   ...
   generate:manifests`).
4. From `plugins/my-plugin`, run `node ../../scripts/validate-plugin.js .` to
