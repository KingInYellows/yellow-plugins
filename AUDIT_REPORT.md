...
| H-04 | S4 | **Historical/deleted:** `plugins/yellow-ci/hooks/hooks.json`, `plugins/yellow-debt/hooks/hooks.json`, `plugins/yellow-ruvector/hooks/hooks.json`, `plugins/yellow-morph/hooks/hooks.json`, `plugins/gt-workflow/hooks/hooks.json`, `plugins/github-workflow/hooks/hooks.json` | All six plugins shipped a `hooks/hooks.json` reference file alongside the inline `plugin.json.hooks` block. MEMORY.md notes the file is reference-only since 2026; only some carried the `_comment` annotation. | **Resolved 2026-09-15:** mirrors deleted (Claude Code auto-loaded them, double-registering hooks); RULE 7 now errors on any `hooks/hooks.json` presence. Original recommendation: spot-check each `hooks/hooks.json` for the `"_comment"` key and timeout-drift against `plugin.json`. (yellow-morph's `hooks/hooks.json` was previously confirmed non-authoritative.) | S |
