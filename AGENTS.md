...
- `pnpm generate:manifests`: Apply mode — regenerate root
  `.claude-plugin/marketplace.json` and `.agents/plugins/marketplace.json`,
  each plugin's `.claude-plugin/plugin.json`, plus Codex/Cursor outputs
  (`plugins/<name>/.codex-plugin/plugin.json`, `hooks/codex-hooks.json`,
  `codex/skills/`, `plugins/<name>/.cursor-plugin/plugin.json`,
  `cursor/skills/`, root `.cursor-plugin/marketplace.json`). Manifests and
  exposure allowlists come from `catalog/`; Codex/Cursor skill bodies are
  copied from `plugins/<name>/skills/<skill>/` (the catalog only selects
  which skills are exposed). Never hand-edit those generated files — edit
  the catalog for manifests/allowlists, or the plugin skill source for
  skill bodies, then regenerate.
...
- `pnpm validate:versions` / `pnpm validate:versions:dry`: Check three-way
  ...
  `marketplace.json`, plus two-way checks of `package.json` against
  `.codex-plugin/plugin.json` and `.cursor-plugin/plugin.json` when those
  targets exist.
...
- Per-plugin `package.json` is the version source of truth.
  ...
  `.claude-plugin/marketplace.json`. Codex/Cursor plugin manifests are
  regenerated by `pnpm generate:manifests` from the same `package.json`.
