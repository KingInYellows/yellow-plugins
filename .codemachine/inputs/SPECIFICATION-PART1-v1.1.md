...
- **Changes from v1.0.0**: Applied corrections from adversarial review (CRIT-001, CRIT-003, CRIT-004, CRIT-010, CRIT-011)
...
**Step 9 (Download and Validation)**:
- App downloads plugin files to cache:
  - Download to staging: `~/.claude/plugins/staging/{id}/` → app **MUST** download all files
  - Validate entrypoints exist → app **MUST** verify all entrypoint files exist (MOVED UP for fail-fast)
...
- Total install time: p95 ≤ 2 minutes (NFR-PERF-001, for typical plugins < 5 MB)
...
- [ ] Given plugin already installed, When user attempts install, Then conflict resolution flow triggers
...
Permissions:
  - filesystem
    Reason: Read conversation history to analyze unwanted behaviors and write hook configuration files
  - shell
...
- [ ] Given plugin with permissions, When user views details, Then permissions shown with warning about disclosure-only model
...
**Performance Requirements**:
- CI validation time: < 5 minutes (NFR-MAINT-002, validation only, excludes plugin tests)
...
- `category` (REQUIRED, enum: development, productivity, security, learning, testing, design, database, deployment, monitoring, other)
...
- category must be one of 10 official values (including "other")
...
- `compatibility` (REQUIRED, object, compatibility requirements)
  ...
  - `nodeMax` (OPTIONAL, major version string "18"-"24", maximum Node.js) **NEW in v1.1 - CRIT-019**
...
- nodeMin/nodeMax must be 18-24 (NOT 25+)
...
- `plugins` (REQUIRED, object, key = plugin ID, value = PluginConfig)
  ...
  - `previousVersions` (REQUIRED, array of semver, rollback targets - keeps last 3 versions)
...
- previousVersions must exist in cache directory (limited to last 3 versions per plugin)
...
#### ERROR-COMPAT-002b: Node.js Version Too New (NEW in v1.1 - CRIT-019)
**Trigger**: Current Node.js version > nodeMax
**Behavior**: App **MUST** block install
**Message**:
```
...
[REASON]: Plugin requires filesystem permission
...
[FIX]: Use official category from enum:
  Valid: development, productivity, security, learning, testing,
         design, database, deployment, monitoring, other
...
| 2.0 User Journeys | PRD 5.1-5.4, Discovery D01, G01 | 100% (10 journeys, added uninstall) |
...
## Appendix B: Changes in v1.1

**Critical Corrections Applied**:
1. **CRIT-001**: Atomic transaction boundaries (See Part 2)
2. **CRIT-003**: Permission disclosure model clarified (Section 3.3)
3. **CRIT-004**: Install script security warnings (Section 2.2.1)
4. **CRIT-010**: Plugin conflict resolution (Section 2.2.1, Step 8)
5. **CRIT-011**: Uninstall user journey (Section 2.2.10 - NEW)

**Additional Improvements**:
- Added nodeMax field for Node.js version constraints (CRIT-019)
- Clarified changelog display behavior (CRIT-008, Section 2.2.2)
- Enhanced permission disclosure warnings (Section 2.2.6)
- Improved search behavior specification (Section 2.2.5)
- Added category "other" for uncategorizable plugins

**Document Status**: FINAL (Post-Adversarial Review) ✓
**Word Count**: ~14,500 words
**Ready for Implementation**: YES
**Memory Storage**: `search/synthesis/specification-part1-v1.1`
