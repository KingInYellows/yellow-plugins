...
Detailed catalog of self-hosted runner failure patterns for the failure-analyst agent.
...
2. Add swap: `sudo fallocate -l 4G /swapfile && sudo mkswap /swapfile && sudo swapon /swapfile`
3. Increase VM memory in Proxmox
4. For Node.js: `NODE_OPTIONS=--max-old-space-size=4096`

**Correlation:** Check runner memory with `/ci:runner-health` — if memory <90%, failure was transient spike.
...
**Correlation:** Check disk with `/ci:runner-health` — if >85%, cleanup recommended.
...
**Severity:** Medium | **Frequency:** Frequent | **Auto-recoverable:** Yes (on retry)
