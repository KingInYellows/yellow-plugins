...
13+ regex patterns applied by `lib/redact.sh` before displaying any CI log content.
...
1. **Shell pre-filter:** `lib/redact.sh` applied to all log content via `sed` pipeline
2. **Agent re-check:** Failure-analyst agent instructed to never display unredacted content
3. **Post-output warning:** Always append "Review diagnosis output for sensitive data before sharing"
...
**Fence marker escaping:** Before wrapping, replace any `--- begin` or `--- end` in log content with `[ESCAPED] begin` / `[ESCAPED] end` to prevent injection.
...
### SSH Host
- IPv4: Must be in private range (10.x.x.x, 172.16-31.x.x, 192.168.x.x, 127.x.x.x)
...
Validate ALL inputs before SSH. Reject shell metacharacters (`;`, `&`, `|`, `$(`, backticks).
