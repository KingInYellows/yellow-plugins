...
Rules for detecting common issues in GitHub Actions workflow files for
self-hosted runners.
...
**Detection:** Steps with `npm install`, `pip install`, `cargo build`,
`go build` without preceding cache step.
...
**Detection:** Strings containing `/home/runner/work/`,
...
**Detection:** Workflow triggered by `pull_request` without top-level
`concurrency` key.
...
**Detection:** Jobs using Docker (`docker run`, `docker-compose`) without
cleanup step.
...
**Detection:** Steps named with "cleanup", "teardown", "clean" without
`if: always()`.
