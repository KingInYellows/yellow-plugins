...
# routing_rules items are the one exception with quoting support: the setup
# skill always writes them as a double-quoted scalar (escaping `\` and `"`),
# and rt_extract_rules()/rt_unquote_rule_stream() below unwrap that quoting —
# see the comment on rt_unquote_rule_stream() for why a single left-to-right
# scan is required instead of two global substitutions.
#
