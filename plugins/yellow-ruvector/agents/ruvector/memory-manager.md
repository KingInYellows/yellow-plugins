...
If `hooks_remember` fails, returns an error, or returns a result that does not acknowledge persistence: log '[memory-manager] Failed to store entry: <error>. Entry not saved.' Output `**Stored**: false — <error summary>` so callers can detect the failure. An empty result is "persistence not acknowledged", not saved. Do not retry.
...
6. For `file_change` entries: re-index via
   ...
   for the next flush; if it succeeds and the result acknowledges the
   write, treat them all as processed. A success-shaped result that does
   not acknowledge persistence is a failed store, not a saved one, and
   those entries stay retained
