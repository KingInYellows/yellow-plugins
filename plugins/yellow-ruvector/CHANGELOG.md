...
- [`9d22360`](https://github.com/KingInYellows/yellow-plugins/commit/9d22360ea4908164bc19babb3c99e26820467fce)
  Thanks [@KingInYellow18](https://github.com/KingInYellow18)! -
  `/ruvector:status` provenance check: a dry-run that exits 137 is now reported
  as `UNKNOWN` with SIGKILL wording that names both possible causes (the 5 s
  `--kill-after` grace after an ignored TERM, or an external signal such as the
  OOM killer) instead of asserting the 90 s deadline elapsed. The
  ...
  `normalize`, `prefixPolicy`), projecting both sides by plain key indexing (a
  present `false` is kept; missing and explicit `null` compare equal), so an
  ...
  `npx`. The ADR-210 solution doc gains a "Non-atomic store write race" section
  (symptom, verified ruvector 0.2.34 / 0.2.41 / 0.3.1 write sites, recovery from
  the `.corrupt-<epoch>` quarantine, avoidance).
