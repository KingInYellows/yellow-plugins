...
# Verify ruvector CLI is available
...
recent_learnings=$(npx ruvector hooks recall --top-k 3 "recent mistakes and fixes" 2>/dev/null) || {
  printf '[ruvector] Failed to retrieve learnings\n' >&2
  recent_learnings=""
}
skill_learnings=$(npx ruvector hooks recall --top-k 2 "useful patterns and techniques" 2>/dev/null) || {
