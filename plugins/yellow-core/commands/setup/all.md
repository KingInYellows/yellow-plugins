...
Define `composio_mcp_visible` as:
`mcp__plugin_yellow-composio_composio-server__COMPOSIO_SEARCH_TOOLS` is
visible in the Step 1.5 probe results, OR
`mcp__claude_ai_composio__COMPOSIO_SEARCH_TOOLS` is visible, OR
`mcp__composio-server__COMPOSIO_SEARCH_TOOLS` is visible (legacy prefixes —
do not mark NEEDS SETUP when only the prefix differs).

- READY: `composio_mcp_visible` AND `.claude/composio-usage.json` exists.
  `jq` missing is degraded usage tracking, not a setup failure.
- PARTIAL: `composio_mcp_visible` AND the usage counter
  (`.claude/composio-usage.json`) is missing → run `/composio:setup`.
- NEEDS SETUP: not `composio_mcp_visible`. Open `/mcp`, select
