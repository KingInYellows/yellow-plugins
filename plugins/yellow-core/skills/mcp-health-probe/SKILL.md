...
- **Separate "reachable" from "authenticated."** Upstream API probes
  ...
  their token.
- **Skip-probe is HEALTHY, not DEGRADED.** Since userConfig-only
  credentials can't be read from the shell, running the probe is
  impossible. MCP tool visibility already implies the server
  authenticated successfully at startup, so there's no information-gain
  in failing the probe on that path.
