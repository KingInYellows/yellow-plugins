...
form). When a heading's own text contains a literal ` > `, escape it as
` \> ` in the scope string and double any `\` (`## A > B` under `# Parent`
is `Parent > A \> B`). Both feed the review-findings ledger's identity key; the orchestrator defaults a
