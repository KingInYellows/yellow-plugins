...
| `cli-readiness-reviewer` | `yellow-review:review:cli-readiness-reviewer` | cli-readiness | Diff touches `plugins/*/.claude-plugin/plugin.json`, `plugins/*/agents/**/*.md`, `plugins/*/commands/**/*.md`, `plugins/*/skills/**/SKILL.md`, or `plugins/*/hooks/` (concerns disjoint from `plugin-contract-reviewer`: structurally-correct-new-files vs. renames-breaking-callers) |
| `agent-cli-readiness-reviewer` | `yellow-review:review:agent-cli-readiness-reviewer` | agent-cli-readiness | Diff touches `plugins/*/.claude-plugin/plugin.json`, `plugins/*/agents/**/*.md`, `plugins/*/commands/**/*.md`, `plugins/*/skills/**/SKILL.md`, or `plugins/*/hooks/`. Co-fires with `cli-readiness-reviewer` intentionally — `cli-readiness-reviewer` flags impact-per-issue (interactive prompts, ANSI in pipes, unbounded output) while this reviewer applies the 7-principle Blocker/Friction/Optimization rubric (non-interactive defaults, structured output, actionable errors, safe retries, bounded output, composability, discoverability). Step 6 dedup on `normalize(title)` collapses overlap; remaining findings are complementary not duplicate. To narrow to one, set `reviewer_set.exclude` in `yellow-plugins.local.md`. |
| `agent-native-reviewer` | `yellow-review:review:agent-native-reviewer` | agent-native | Diff touches `plugins/*/.claude-plugin/plugin.json`, `plugins/*/agents/**/*.md`, `plugins/*/commands/**/*.md`, `plugins/*/skills/**/SKILL.md`, or `plugins/*/hooks/` (action/context parity, shared workspace, primitives-over-workflows, dynamic context injection) |
...
6. A `<file-line-counts>` block — **only into `thermonuclear-reviewer`,
   ...
   # and phantom crossings with full confidence. An unresolved merge-base
   # must stop the block: with MERGE_BASE empty, `$MERGE_BASE:$base_path`
   # becomes `:path`, which git reads as the INDEX, so every base count
   # would silently come from staged content instead of a commit.
   ...
   # from an empty diff. `>|` writes through `noclobber` (zsh and bash), which
   ...
   while IFS= read -r -d '' rec; do
     ...
     # `git` and `awk` then vanish and every row silently degrades to
     ...
     # the old path and `head` on the new one. A stream that ends mid-record
     # is truncated: `exit`, never `break`, so rows already staged are never
     ...
     # at the 1000/1001 boundary. Read through a file, not a pipe: with
     ...
     rows=$((rows + 1))
     # Bound the block. Past this many measurable files the fence costs the
     # persona more context than the crossing rule can repay; omit it
     # entirely rather than truncate, so it is never partial.
     if [ "$rows" -gt 500 ]; then
   ...
   emits NUL-terminated records with literal (unquoted) paths, which is what
   lets a path containing a quote, backslash, non-ASCII byte, or newline
...
4. Resolve the active stacked-PR provider first: invoke the `Skill` tool
   with `skill: "stack-provider-router"` and read `state` from its result.
   `READY_GRAPHITE` continues with the Graphite push commands below;
   `READY_GITHUB` skips them and uses the GitHub push commands instead; any
...
Graphite push commands (`READY_GRAPHITE`):
...
GitHub push commands (`READY_GITHUB`):
