...
description:
  ...
  need Devin API context, session management patterns, or error handling
  guidance.
...
Reference patterns and conventions for Devin.AI integration workflows. Loaded by
commands and agents for consistent behavior.

## When to Use

Use when yellow-devin plugin commands or agents need shared Devin workflow
context, including API patterns, session management conventions, input
...
All REST API calls target `https://api.devin.ai/v1/`. Authentication via Bearer
token from `DEVIN_API_TOKEN` env var.
...
**Timeouts by operation:** | Operation | --max-time | --connect-timeout |
|-----------|-----------|-------------------| | Session creation | 60 | 5 | |
Other mutations | 30 | 5 | | Status polls | 10 | 5 |
...
**On validation failure:** Report the actual count vs maximum. Never silently
truncate.
...
Operations that terminate sessions or consume significant resources require
