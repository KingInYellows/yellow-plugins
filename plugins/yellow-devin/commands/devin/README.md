...
| `/devin:wiki`       | Query DeepWiki or Devin Wiki about a repository's architecture and patterns. DeepWiki's canonical home is now `yellow-research` — the DeepWiki fallback path discovers its tools at runtime; the primary Devin Wiki path is unaffected. | `<question> [--repo owner/repo]`                          | `/devin:wiki How does authentication work --repo acme/backend`              |
