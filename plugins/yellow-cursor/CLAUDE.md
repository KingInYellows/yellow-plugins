...
`pnpm --filter yellow-cursor run test` (`vitest run --dir tests`; the root
`pnpm test:unit` runs it too), no network. `tests/fake-sdk.ts` implements the
...
`pnpm --filter yellow-cursor run typecheck` (`tsc --noEmit`) is wired into the
root `pnpm typecheck`. `package.json` pins `engines.node` to
`>=22.22.0 <25.0.0`. The version lives in three manifests — `package.json`
(source of truth), `.claude-plugin/plugin.json`, and `.cursor-plugin/plugin.json`
(the Cursor-native twin exposing `cursor/skills/cursor-delegation/SKILL.md`) —
the latter two are regenerated by `pnpm generate:manifests`.
