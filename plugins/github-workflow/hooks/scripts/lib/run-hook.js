...
// Bound stdin to 64KB — a verbose Bash tool_result must not make the hook
// buffer unbounded input or blow its execution timeout. Mirrors
// gt-workflow's identical cap.
 ...
 *
 * Mirrors gt-workflow's run-hook.js exactly (see that file's comment for
 * the truncation/malformed-JSON fail-open/fail-closed rationale) — kept as
 * an independent copy rather than a cross-plugin require so
 * github-workflow has no runtime dependency on gt-workflow being present.
 */
