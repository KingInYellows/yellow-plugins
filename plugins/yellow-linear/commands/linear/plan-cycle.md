...
- If `$ARGUMENTS` provided, validate: alphanumeric, spaces, and hyphens only,
  max 100 characters. Match by cycle name.

If no active cycle exists:

- Report "No active cycle found for this team"
- Suggest creating one manually in Linear's UI (cycle creation is not available
  via MCP)
- Stop
...
1. **Validate ownership (C1):** Call `get_issue` to verify the issue exists and
   ...
   warn.
2. **Detect concurrent changes (H1):** Compare re-fetched state (status, cycle
