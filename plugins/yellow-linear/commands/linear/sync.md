...
Load issue context, link PRs, and update status for the current branch's Linear issue.
...
If validation fails, report the error and stop. Do NOT proceed with writes against an unverified issue ID.
...
- **If PR exists:** Add PR URL to the Linear issue as a comment via `mcp__plugin_linear_linear__create_comment`:
  ...
  ```
- **If no PR:** Note that no PR exists yet. Suggest creating one with `gt submit`.
...
Present the suggestion via AskUserQuestion and let user confirm or choose a different status. Apply via `update_issue` only after confirmation.
