...
- End with the recommended next step: `/flow:work <path>` to execute the stack, or `gt submit --no-interactive` for standalone changes.
...
- When saving decomposition to a plan file, use the structured format defined
  ...
  is the machine-readable contract consumed by `flow:work`.
