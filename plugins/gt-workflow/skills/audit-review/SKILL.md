...
**single message**, one agent per prompt, each with
`subagent_type: general-purpose`, passing `$DIFF_OUTPUT` as context.
...
> verification (e.g. `/codex:rescue`-style live testing) rather than
