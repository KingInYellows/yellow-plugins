...
1. **Circular dependencies causing build failures** → Critical
   ...
   **Circular dependency detection — use in priority order:**
   ...
   Note: Build commands (go build, cargo build) may execute build scripts. Only run them on codebases known to be trusted; for untrusted or unfamiliar code, use the non-build options (steps 2-4) instead.
