...
- [`fbe9c04`](https://github.com/KingInYellows/yellow-plugins/commit/fbe9c04177548a289ecc6b1ff72789e84b9923d9)
  Thanks [@KingInYellow18](https://github.com/KingInYellow18)! - `README.md`
  gains a "Model Selection" section: no `-m` by default, resolution is
  `CODEX_MODEL` → `~/.codex/config.toml` `model` → account default,
  `CODEX_SMOKE_MODEL` affects `/codex:setup` only, and what the
  ...
  is extracted uncapped and the 500-byte `head -c` runs after the redaction awk
  (matching `/codex:rescue` and `/codex:setup`), so a credential that straddled
  the old 400-byte cut can no longer surface as an unredacted fragment. The
  plugin's first bats suite, `tests/redaction.bats`, pins that order by
  extracting the awk program from `review.md` at run time.
