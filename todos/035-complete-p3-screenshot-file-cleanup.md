...
## Problem Statement
agent-browser screenshots accumulate in test-reports/ with no cleanup mechanism. Over time could consume significant disk space, especially for projects with frequent test runs or long-running test suites.

## Findings
- Files: test-reports/*.png
- Every browser interaction that takes a screenshot leaves a file in test-reports/
...
## Recommended Action
Implement Option A with 7-day retention. Add cleanup step to both test and explore commands before agent spawn. Document retention policy in test-conventions skill.
