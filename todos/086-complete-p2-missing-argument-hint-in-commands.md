...
## Problem Statement
17 of 48 commands across multiple plugins are missing the `argument-hint` field in their frontmatter. This creates inconsistency in the command interface and reduces discoverability of expected argument formats.
...
- Set to a descriptive string for commands that accept arguments (e.g., `"<issue-number>"`, `"<pr-url>"`)
...
## Recommended Action
Adopt Solution 1: add `argument-hint` to all 17 commands with appropriate values.
