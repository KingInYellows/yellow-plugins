...
The session-start.sh script uses `|| true` to suppress cache write failures,
which silently drops errors. If the cache consistently fails to write (due to
permissions, disk full, etc.), the hook will re-fetch from GitHub API every
session, potentially exhausting rate limits. Additionally, leftover `.tmp` files
accumulate on disk.
...
2. **Cascading Problems:**
   - If cache write always fails, script re-fetches from GitHub API every
     session
...
Replace `|| true` with proper error handling that logs failures and cleans up
temporary files.
