...
Error severity classification (critical/high/medium/low) defined in both
test-conventions skill and test-reporter agent. Could diverge if one is updated
without the other, leading to inconsistent error classification across the
plugin.
...
- If criteria change (e.g., elevate a11y issues to high), both files must be
  updated
...
- test-reporter.md references the skill: "Use severity levels from
...
Implement Option A. Remove severity table from test-reporter.md. Add reference
to test-conventions skill. Keep detailed severity criteria only in the skill.
This reduces risk of divergence and makes it clear where to update severity
definitions.
...
Classify errors by severity using the criteria defined in the test-conventions
skill:
