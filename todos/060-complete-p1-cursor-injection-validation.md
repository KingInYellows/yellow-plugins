...
The `get-pr-comments` script uses cursor values from GitHub GraphQL responses
directly in shell variables without validating their format. A malicious or
malformed cursor could inject shell metacharacters.
...
- **Evidence:** `CURSOR` variable is set from `jq -r '.data...endCursor'` output
  and used in subsequent GraphQL query string construction without format
  validation
...
- Build the entire GraphQL query JSON via `jq -n --arg` instead of string
  interpolation
...
- [ ] Cursor values validated against safe character pattern before use in
      queries
