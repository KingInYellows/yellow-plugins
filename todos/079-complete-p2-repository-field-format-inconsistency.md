...
The `repository` field in `plugin.json` files uses inconsistent formats across
the marketplace. Two plugins (gt-workflow and yellow-core) use simple string
format, while seven others use the object format with
`{"type": "git", "url": "..."}`. Additionally, gt-workflow is missing the
`homepage` field.
...
Convert gt-workflow and yellow-core to use `{"type": "git", "url": "..."}`
format and add homepage field to gt-workflow.
...
1. `plugins/gt-workflow/.claude-plugin/plugin.json` - convert repository to
   object, add homepage
2. `plugins/yellow-core/.claude-plugin/plugin.json` - convert repository to
   object
