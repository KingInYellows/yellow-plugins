...
The install script installs `ruvector` without version pinning by default. While
`--ignore-scripts` mitigates post-install attacks, malicious code in the package
itself executes when `npx ruvector` runs.

**Why it matters:** Supply chain compromise, breaking changes between versions,
non-reproducible installs.

## Findings

- **Security Sentinel (H4):** No default version pin, --ignore-scripts only
  partial mitigation
