...
The yellow-ci plugin uses `"timeout": 3000` (implying milliseconds) while
yellow-ruvector uses `"timeout": 3` (implying seconds). This inconsistency
creates confusion and potential runtime issues if the timeout unit is
misinterpreted.
...
- If milliseconds: yellow-ci is correct (3000 = 3s), update yellow-ruvector to
  3000
...
- Add comment in each hooks.json: `// timeout in milliseconds` or
  `// timeout in seconds`
...
**Recommendation:** If specification is ambiguous, prefer milliseconds for
consistency with common JavaScript/TypeScript conventions and finer-grained
control.
