...
`validate_ssh_host()` spawns 3 subprocesses per call (grep + 2x cut) for IPv4 validation. For multi-runner operations like `/ci:runner-health` with 10+ runners, this creates 30+ unnecessary fork/exec cycles.

## Findings

- **File**: `plugins/yellow-ci/hooks/scripts/lib/validate.sh:222-227`
- **Current**: `printf | grep` for IPv4 detection, `printf | cut` for octet extraction (3 subprocesses)
