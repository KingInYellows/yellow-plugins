...
## Problem Statement
The test-runner agent performs login to the application but has no verification step to confirm login succeeded before proceeding with tests. If login fails silently, all subsequent tests would fail with confusing errors instead of clear "authentication failed" message.

## Findings
- **File affected**: `agents/testing/test-runner.md`
- **Current behavior**: Login flow executed but success not verified
- **Failure mode**: Failed login → cascade of test failures with misleading errors
...
This provides defense in depth - both artifact checking and behavioral verification.
...
- **Protected route**: Use first route from test suite or dedicated auth-check endpoint
- **Error handling**: Should abort all testing, not attempt to run tests unauthenticated
...
- [ ] Manual test: provide wrong credentials, verify fast failure with correct error
