...
## Problem Statement
The `yellow-debt/lib/validate.sh` script uses GNU-specific `realpath -m` flag at line 74, which is not available on macOS/BSD systems. This creates a portability issue, while `yellow-ruvector` demonstrates a portable fallback pattern using `cd`+`pwd`.
...
The `-m` flag (allow missing components) is GNU-specific and not available in BSD/macOS realpath.
...
Implement a portable canonicalization function that works on both GNU/Linux and macOS/BSD.
...
- `plugins/yellow-ruvector/hooks/scripts/lib/validate.sh` (portable reference implementation)
