...
## Problem Statement
The `gt-workflow/commands/smart-submit.md` command auto-proceeds to `gt submit` when the audit is clean or contains only minor issues, violating the project's human-in-the-loop security requirement that "commands that push LLM-generated code must AskUserQuestion before gt submit."
...
This violates Agent Workflow Security Patterns from project memory, which explicitly requires human approval before any git submission of LLM-generated changes.
...
This ensures compliance with: "commands that push LLM-generated code must AskUserQuestion before gt submit"
