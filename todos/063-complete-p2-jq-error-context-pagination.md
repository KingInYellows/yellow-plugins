...
The `get-pr-comments` pagination loop pipes GraphQL responses through jq but
doesn't capture stderr. If jq fails to parse a response (malformed JSON,
unexpected schema), the error is silently lost.
...
- **Evidence:** `jq` stderr not captured; parse failures produce empty output
  with no diagnostic
...
- Redirect jq stderr to a variable, log on failure with
  `[get-pr-comments] Error:` prefix
