...
The `get-pr-comments` pagination loop accumulates results using `jq -s '.[0] + .[1]'` on each iteration, causing O(n²) performance as each merge re-parses all previously accumulated JSON.
...
- **Evidence:** Each page merge re-serializes all previous pages. For PRs with 10+ pages of review threads, this causes noticeable slowdown.
