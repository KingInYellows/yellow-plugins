...
The `install-agent-browser.sh` script uses `npm install -g agent-browser` without version pinning. A compromised package version could execute arbitrary code on user machines during installation, representing a critical supply chain security risk.
...
The npm install process runs arbitrary scripts (preinstall, postinstall) which execute with the user's permissions, making this a high-severity attack vector.
...
**Current code location:** `plugins/yellow-browser-test/scripts/install-agent-browser.sh`
...
- [ ] Version compatibility matrix added (agent-browser version → Chromium version)
