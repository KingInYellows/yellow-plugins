...
`project-standards-reviewer`) and on `learnings-researcher` (the Step 3d pre-pass),
the agents that run with a stable system prompt on every review. The one-hour prompt
