...
2. **Subagent Failure Convention (Output-File Pattern)** — documents the
   ...
   (GitHub Issues #24181, #25818): orchestrators create a per-run temp
   directory via `mktemp -d -t <prefix>-XXXXXXXX`, spawned agents write a
   structured JSON result file to `$RUN_DIR/agent-result-<agent>.json`,
   and orchestrators read the file rather than relying on stdout parsing.
