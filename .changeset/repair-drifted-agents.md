...
The 3 new review agents and the 2 modified review agents (performance-oracle, security-sentinel) are read-only (`tools: [Read, Grep, Glob]`) per the W1.2 read-only-reviewer rule. The 4 research/workflow agents retain their existing tool sets (Bash, WebSearch, etc.). The 3 new reviewers will be wired into the W2.4 review:pr orchestrator dispatch table.
