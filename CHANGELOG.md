...
All notable changes to the Yellow Plugins project will be documented in this file.
...
### Added
- **Domain Services**: Complete plugin lifecycle orchestration (FR-004, FR-007, FR-009)
...
- **Infrastructure Layer**: Comprehensive support services
  ...
  - Compatibility policy engine with 5-dimension validation (FR-005, NFR-REL-003)
...
- **Observability & Metrics**: Structured telemetry stack (Section 6)
  - Metrics exporter with KPI tracking (install success, rollback duration, cache size)
...
- **CI/CD Automation**: GitHub Actions workflows
  - `validate-schemas.yml` - Schema validation with 10 marketplace + 12 plugin rules (< 1min)
  - `publish-release.yml` - Multi-stage release workflow with validation, SBOM, and artifact generation
  - Release workflow supports both tag-triggered and manual dispatch modes
- **Operational Documentation**: (Section 3.7, Section 3.16)
  - `docs/operations/feature-flags.md` - Feature flag governance and precedence rules
...
All flags default to **disabled** unless explicitly enabled (Section 4 directive):
...
- `enableCompatibilityChecks`: **true** - Safety-critical validation (NFR-REL-003)
...
- **Layer Boundaries**: ESLint enforces clean architecture (domain ← infrastructure ← cli)
...
- Lifecycle script confirmation with typed consent (`"I TRUST THIS SCRIPT"`) (Section 4)
...
- **Iteration 4 Tasks**: I4.T1 (Publish), I4.T2 (CI Expansion), I4.T3 (Metrics), I4.T4 (Runbooks)
...
### Known Limitations
- Permission enforcement is **disclosure-only** in Phase 1 (FR-012 clarification)
...
- **User Journeys**: 10 complete user workflows documented (Section 2.2)
  ...
  - Check for updates
  - Uninstall plugin
- **Error Scenarios**: 23 documented error cases with recovery paths (Section 4.3)
- **Quality Assurance**:
...
- Local-first caching with `.claude-plugin/` directory (ADR implied in Section 3.2)
...
### Success Metrics
- **PSM**: Install plugin in ≤ 2 minutes (p95) with 4-dimension compatibility check
- **SSM-1**: 100% rollback success without manual cleanup
- **SSM-2**: Publish new plugin in ≤ 10 minutes from tag push to marketplace entry
...
For changelog automation, the GitHub Actions workflow extracts release notes using:
```bash
