...
3. **Validate OpenCode bare-slug `--model` invocation at PR-D time.** The slug `deepseek/deepseek-v4-pro` is user-confirmed on OpenRouter (live as of 2026-05-08), but OpenCode's `--model` flag with this exact bare slug + `defaultProvider: "openrouter"` config has not been spiked in this codebase. Run `opencode run --model deepseek/deepseek-v4-pro "test"` once before PR-D ships, update `docs/spikes/opencode-cli-format-json-2026-05-04.md` with the result. If invocation fails, fix the OpenCode `defaultProvider` config (`opencode.json`) or fall back to an explicit `--provider openrouter --model deepseek/deepseek-v4-pro` shape if OpenCode supports it. Do NOT switch to a 3-segment `--model openrouter/deepseek/deepseek-v4-pro` slug — provider routing is handled by `opencode.json`, not by the model slug, per Task 4.1's locked-decision body and the deepen-plan annotation above.
