...
- **Atomic settings.json write.** Bind a temp path via `mktemp` and reference it consistently across `jq`, `chmod`, and `mv`: `tmp=$(mktemp ~/.claude/settings.json.tmp.XXXXXX) && jq '...' ~/.claude/settings.json > "$tmp" && chmod --reference=~/.claude/settings.json "$tmp" && mv "$tmp" ~/.claude/settings.json`. `mktemp` (over a bare `$$` PID suffix) avoids collision races on concurrent `/rtk:setup` runs. **Permissions are NOT preserved by `mv` / `rename(2)`** — the destination inherits the temp file's `umask`-based mode (typically 0600 from `mktemp`). The `chmod --reference=...` step (Linux) or `chmod $(stat -f '%Mp%Lp' ~/.claude/settings.json) "$tmp"` (macOS) **before** the `mv` restores the original mode. Capture the original mode immediately before any mutation, since RTK's own `rtk init` may have already set the file to 0600. See Phase 3 Step 5 for the canonical implementation block.
