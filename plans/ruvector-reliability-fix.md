...
3. **No retry logic** means a single MCP failure skips memory operations
   entirely
...
> first" is the emerging de facto best practice for absorbing cold start. See:
...
- `ruvector` is not installed globally; all calls go through `npx` (1,869ms
  overhead)
- `install.sh` verifies with `npx ruvector --version` (false positive for global
  binary)
...
- All 7 consuming workflow entrypoints rely on inline recall/remember logic (no
  `<skill>` tag mechanism)
- `hooks_capabilities` is not used anywhere in the codebase

<!-- deepen-plan: codebase -->

> **Codebase:** All plan claims about current state are accurate after including
> `review-all.md` in the consuming-command count. `install.sh` line 121 uses
> `npx ruvector --version`; `setup.md` lines 99-101 and 127-129 produce warning
> text only; SKILL.md Patterns 1 and 2 both skip immediately on MCP error with
> no retry. `hooks_capabilities` returns zero matches across the entire codebase
> outside plan/brainstorm docs.
...
2. **Layer 2 (Warmup):** Add `hooks_capabilities` warmup call to absorb MCP cold
   start before real recall/remember calls
...
- [ ] 1.1: Update `plugins/yellow-ruvector/scripts/install.sh`
  - After `npm install -g ruvector --ignore-scripts`, verify with
    `command -v ruvector` (not `npx`)
  - On permission failure, retry with `--prefix ~/.local`
  - If the fallback install lands in `~/.local/bin` and that path is not in
    PATH, print exact shell-profile guidance for the user
  - Export PATH only inside `install.sh` as needed to finish verification in the
    subshell; do not rely on that export reaching the parent session
  - Final verification: `command -v ruvector` must succeed or script exits 1
  - Replace line 121 `npx ruvector --version` verification with
    `ruvector --version`

<!-- deepen-plan: codebase -->

> **Codebase:** Confirmed install.sh lines 102-114 already try
> `--prefix ~/.local` on permission failure and warn about PATH, but do NOT
> modify the shell profile automatically. The script warns:
> `"Installed to ~/.local — ensure ~/.local/bin is in your PATH"` and
> `"Add to your shell profile: export PATH=\"$HOME/.local/bin:$PATH\""`. The fix
> should use `npm prefix -g` plus `/bin` to detect the actual global install
> directory rather than relying on deprecated `npm bin -g`.

<!-- /deepen-plan -->

<!-- deepen-plan: external -->

> **Research:** Use `npm prefix -g` and append `/bin` to detect where global
> binaries are installed. For the `--prefix ~/.local` fallback, print exact
> shell-profile guidance rather than mutating rc files automatically. Detect the
> user's shell via `$SHELL` basename (with `$ZSH_VERSION` / `$BASH_VERSION` as
> fallback signals), and use an explicit PATH membership check or `grep -q`
> before emitting duplicate guidance.
...
> `bash "${CLAUDE_PLUGIN_ROOT}/scripts/install.sh"` from setup.md. Any
> `export PATH` in install.sh will NOT propagate to the parent session. The
> install script can only use `export PATH` to complete verification inside that
> subshell. The user-visible remediation must still be to print the exact
> rc-file guidance and stop setup if `command -v ruvector` remains unavailable
...
> with these managers. If `$NVM_DIR` is set, or if `$FNM_DIR` is set or
> `command -v fnm` succeeds, warn users that global installs are
> version-specific. The fnm multishell path
...
- [ ] 1.2: Update `plugins/yellow-ruvector/commands/ruvector/setup.md`
  - Step 3 verification: treat missing global binary as a **failure**, not a
    warning
  - Add smoke test: `timeout 1 ruvector hooks recall --top-k 1 "setup-test"`
    must complete within 1s
  - Report table should show "Global binary" as REQUIRED
  - If binary not found after install step, show remediation instructions and
    stop

<!-- deepen-plan: codebase -->

> **Codebase:** Setup convention across the monorepo is "collect-all-then-stop":
> yellow-ci, yellow-morph, and yellow-devin all collect ALL missing
> prerequisites and report together before stopping. yellow-ruvector is the
> outlier — it warns but continues. The fix should follow the established
> collect-then-stop pattern.

<!-- /deepen-plan -->

### Phase 2: Integration Pattern Hardening (Layers 2 & 3)

- [ ] 2.1: Update `plugins/yellow-core/skills/mcp-integration-patterns/SKILL.md`
...
- [ ] 2.2: Update SKILL.md Pattern 2 (Tiered-Remember-After-Act)
  - Add retry-once with 500ms pause to `hooks_remember` execution:
    ...
    wait 500 milliseconds, retry exactly once. If retry fails: note
    `[ruvector] Warning: remember failed after retry — learning not persisted`
    and continue.
    Do NOT retry on validation errors.
    ```
  - Add retry-once to dedup check `hooks_recall` call as well
  - Note: warmup is NOT needed for Pattern 2 if Pattern 1 already ran in the
    same session (MCP server is warm)
...
> Consider adding a "## MCP Tool Reliability" named section to the SKILL.md that
> commands can reference, reducing copy-paste drift over time.
...
- [ ] 3.4: Update `plugins/yellow-core/commands/workflows/compound.md`
  - Add `hooks_capabilities` to allowed-tools
  - Add warmup before Step 3 remember flow (since compound may not have a prior
    recall)
...
> calls conflicts with this instruction. Resolution: the retry-once pattern in
> `mcp-integration-patterns` applies to workflow commands (brainstorm, plan,
...
- [ ] 4.2: Manual smoke test: run `/ruvector:setup` and verify global binary
      install
- [ ] 4.3: Manual smoke test: run `/workflows:brainstorm` on a test topic and
      verify recall succeeds (no "Skipped" message)
- [ ] 4.4: Manual smoke test: run `/workflows:compound` and verify remember
      succeeds
...
| #   | File                                                           | Plugin          | Change                                                 |
| --- | -------------------------------------------------------------- | --------------- | ------------------------------------------------------ |
| 1   | `plugins/yellow-ruvector/scripts/install.sh`                   | yellow-ruvector | `command -v` verification, PATH guidance, hard failure |
...
Three plugins are touched: `yellow-ruvector` (setup), `yellow-core` (patterns +
4 commands), `yellow-review` (3 commands). Each will need a changeset.
...
- Workflows that use recall no longer show "Skipped -- ruvector recall not
  available"
- MCP cold start is absorbed by `hooks_capabilities` warmup before first real
  call
- Transient MCP failures are retried once (500ms pause) before skipping
- All existing recall/remember behavior is preserved (score filtering,
  truncation, XML sanitization)

## Edge Cases

- **npm permissions failure:** `install.sh` falls back to `--prefix ~/.local`
  and prints PATH guidance
- **PATH not updated in current session:** `install.sh` can export PATH only in
  its verification subshell; the user still needs to update their shell profile
  or restart the shell before setup can pass cleanly
- **MCP server never starts:** Warmup call fails, retry fails, workflow skips
  gracefully (existing behavior)
- **ruvector already installed globally:** `install.sh` should detect and skip
  reinstall
- **Warmup succeeds but recall fails:** Retry-once handles this; if retry also
  fails, skip gracefully
...
> `$NVM_DIR`, or detect fnm via `$FNM_DIR` plus `command -v fnm`, and warn
> users. The fnm multishell path (`/run/user/*/fnm_multishells/*/bin`) is
> session-specific and won't persist.
...
>   — initialization handshake, no built-in warmup
> - [npm prefix docs](https://docs.npmjs.com/cli/v10/commands/npm-prefix/) —
>   supported global prefix detection
> - [npm folders docs](https://docs.npmjs.com/cli/v10/configuring-npm/folders/)
>   — prefix configuration hierarchy
> - [MCP Server Performance Benchmark](https://www.tmdevlab.com/mcp-server-performance-benchmark.html)
>   — cold start latency data across languages
> - [AI Agent retry patterns](https://fast.io/resources/ai-agent-retry-patterns/)
>   — wording reliability for retry instructions in agent prompts
