...
- **Item #10 (`feat/optimize-skill`, W3.14) — DONE.** Shipped via PR #311 (squash-merged 2026-05-01 as `9cb0f32` on `main`). Adds `plugins/yellow-core/skills/optimize/SKILL.md` + `schema.yaml` — LLM-as-judge with parallel candidate variants, two-run order-swap (Run-3 downgrades to 2-run when `parallel_count == 2`, surfaced to the user via a one-line note), per-criterion analytic rubric, `style_bias_check` self-flag, paginated 4-option Phase 4 selection (top-2 + Cancel + Other-with-pagination).
