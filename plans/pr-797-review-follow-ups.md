...
- **Deferred P3s from the Phase 3 review pass (2026-09-16)**, all
  ...
  - `generate-manifests` apply mode deletes a hand-authored
    `plugins/*/hooks/hooks.json` (WS1 asked for the stale-sweep entry;
    RULE 7 already errors first in `validate:plugins`). If that ever bites,
  ...
  - Cross-plugin duplication of the `jq -n … systemMessage` idiom
    ...
    yellow-ruvector now has `emit_message_json` in its `hook-json.sh` —
